...
// Inode content
//
// 每个 inode 关联的数据内容存储在磁盘的块（block）中。
// 最前面的 NDIRECT 个块号直接存储在 ip->addrs[] 数组中。
// 接下来的 NINDIRECT 个块号存储在 ip->addrs[NDIRECT] 所指向的间接块（一个块中存储的是块号）。

// 该函数用于返回 inode ip 中第 bn 个数据块对应的磁盘块地址。
// 如果该块还没有被分配（即为 0），bmap 会调用 balloc() 为其分配一个磁盘块。
static uint
